...
$.trafficCop = function(url, options) {
    ...
    var remove = function() {
        ...
        traffic = {
            ...
            success: function() {
                $.each($(inProgress[key].successCallbacks), function(idx,item){ item.apply(null, arguments); });
                remove();
            },
            error: function() {
                $.each($(inProgress[key].errorCallbacks), function(idx,item){ item.apply(null, arguments); });
                remove();
            }
        };
